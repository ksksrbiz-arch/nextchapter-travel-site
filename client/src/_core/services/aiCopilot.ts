...
class AICoPilotService {
  ...
  generateInsights(businessData: any): CopiloitInsight[] {
    ...
    if (businessData.clients) {
      businessData.clients
        .filter((c: any) => c.lastBooking && Date.now() - c.lastBooking > 180 * 24 * 60 * 60 * 1000)
        .forEach((client: any) => {
          insights.push({
            ...
            impact: 'high',
            suggestedAction: 'Send personalized re-engagement email with special offer',
      ...
      businessData.clients
        ...
        .forEach((client: any) => {
          insights.push({
            ...
            impact: 'medium',
            suggestedAction: 'Create luxury upgrade package tailored to their travel style',
            relatedClient: client.id,
            confidence: 0.78,
            createdAt: Date.now(),
          });
    ...
    if (businessData.team) {
      const avgTrips = businessData.team.reduce((sum: number, m: any) => sum + m.performance.tripsManaged, 0) / businessData.team.length;
      const overloaded = businessData.team.filter((m: any) => m.performance.tripsManaged > avgTrips * 1.3);
      ...
      if (overloaded.length > 0) {
        insights.push({
          ...
          impact: 'medium',
          suggestedAction: 'Redistribute upcoming trips or consider hiring support',
          confidence: 0.85,
          createdAt: Date.now(),
        });
      }
    ...
    if (businessData.analytics) {
      ...
      insights.push({
        ...
        impact: 'high',
        suggestedAction: 'Create premium tier offerings (concierge, private guides, exclusive experiences)',
        confidence: 0.88,
        createdAt: Date.now(),
      });
    }
    ...
    if (businessData.analytics?.topDestinations) {
      ...
      insights.push({
        ...
        impact: 'high',
        suggestedAction: 'Develop signature ${topDest.destination} experience package with premium positioning',
        confidence: 0.91,
        createdAt: Date.now(),
      });
    }
   ...
   */
  generateAutomationSuggestions(businessData: any, focusArea?: string): AutomationSuggestion[] {
    ...
    if (businessData.clients) {
      ...
      businessData.clients
        .filter((c: any) => c.status === 'vip' && Date.now() - c.lastBooking > 60 * 24 * 60 * 60 * 1000)
        .forEach((client: any) => {
          suggestions.push({
   ...
   */
  private generateCopilotResponse(userMessage: string, context: ConversationContext): string {
...
Opportunity: Introducing premium amenities could boost average trip value by 25% (+$180K/year). Should we discuss this?`;
    ...
    if (lowerMessage.includes('team') || lowerMessage.includes('staff') || lowerMessage.includes('wendy') || lowerMessage.includes('jessica')) {
...
I recommend prioritizing the satisfaction surveys to gather feedback. Should I execute these tasks?`;
    ...
    if (lowerMessage.includes('goal') || lowerMessage.includes('target') || lowerMessage.includes('forecast')) {
...
What specific area would you like to explore?`;
  ...
  getHealthRecommendations(healthScore: number): string[] {
    ...
    if (healthScore < 50) {
      recommendations.push('⚠️ Health score is concerning. Focus on client satisfaction and issue resolution.');
      recommendations.push('Review team workload distribution and consider support hiring.');
    } else if (healthScore < 70) {
      recommendations.push('✓ Solid foundation. Opportunities: Expand high-performing team members\' portfolios.');
      recommendations.push('Create premium offerings to increase average trip value.');
    } else if (healthScore < 85) {
      recommendations.push('✓ Excellent operations. Explore market expansion to new destinations.');
      recommendations.push('Develop strategic partnerships for exclusive experiences.');
    } else {
      recommendations.push('✓ Outstanding performance! Consider franchise or partnership models.');
      recommendations.push('Invest in technology and innovation for competitive advantage.');
    }
