...
class MemoryArchivesService {
  ...
  getArchivesByDestination(destination: string): ArchivedMemory[] {
    return Array.from(this.archives.values()).filter(
      m => m.destination.toLowerCase().includes(destination.toLowerCase())
    );
  }
  ...
  generateRecommendations(memory: ArchivedMemory): TripRecommendation[] {
    ...
    const alternatives = [
      { dest: 'Paris', emotion: 'romantic', activities: ['museums', 'dining', 'culture'] },
      { dest: 'Tokyo', emotion: 'adventurous', activities: ['exploration', 'food', 'culture'] },
      { dest: 'Costa Rica', emotion: 'adventurous', activities: ['hiking', 'nature', 'wildlife'] },
      { dest: 'Greece', emotion: 'relaxed', activities: ['beach', 'culture', 'hiking'] },
      { dest: 'Thailand', emotion: 'adventurous', activities: ['food', 'beaches', 'temples'] },
    ];
    ...
    recommendations.push({
      ...
      accommodationType: profile.travelStyle === 'luxury' ? 'Luxury Resort' : 'Boutique Hotel',
      images: [
    ...
    recommendations.push({
      ...
      activities: profile.activities,
      accommodationType: profile.travelStyle === 'luxury' ? '5-Star Hotel' : '4-Star Hotel',
      images: [
  ...
  private getActivityKeywords(emotion: string): string[] {
    const keywords: Record<string, string[]> = {
      ...
      surprised: ['shopping', 'discovery', 'unexpected finds', 'unique experiences'],
      relaxed: ['spa', 'beaches', 'wellness', 'nature'],
      adventurous: ['hiking', 'outdoor sports', 'extreme activities', 'exploration'],
      tender: ['family time', 'cultural sites', 'intimate settings', 'connections'],
  ...
  private estimateBudget(memory: ArchivedMemory): number {
    ...
    const mediaRatio = memory.stats.totalMedia / Math.max(memory.stats.duration, 1);
    return Math.round(
  ...
  private determineTravelStyle(memory: ArchivedMemory): string {
    ...
    if (avgMood > 0.80) return 'cultural';
    return 'relaxation';
  }
