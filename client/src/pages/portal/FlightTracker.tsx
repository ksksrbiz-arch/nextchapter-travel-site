...
import { Plane, Plus, Trash2, AlertCircle, CheckCircle2, Clock, MapPin, Calendar } from "lucide-react";
...
const STATUSES = [
  { value: "scheduled", label: "Scheduled", color: "bg-blue-950/20 border-blue-500/20" },
  { value: "boarding", label: "Boarding", color: "bg-purple-950/20 border-purple-500/20" },
  { value: "delayed", label: "Delayed", color: "bg-orange-950/20 border-orange-500/20" },
  { value: "cancelled", label: "Cancelled", color: "bg-red-950/20 border-red-500/20" },
  { value: "landed", label: "Landed", color: "bg-emerald-950/20 border-emerald-500/20" },
];
...
export function FlightTracker() {
  ...
  const validateForm = () => {
    ...
    if (!formData.arrivalAirport) newErrors.arrivalAirport = "Arrival airport required";
  ...
  const upcomingFlights = flights.filter((f) => f.status !== "landed" && f.status !== "cancelled");

  return (
    <div className="space-y-6">
      {/* Quick Stats */}
      <Card className="bg-gradient-to-br from-sky-950/30 to-blue-950/30 border-sky-500/20 p-6">
        <h3 className="text-sm font-medium text-muted-foreground mb-4">Flight Summary</h3>
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <div className="p-3 bg-black/20 rounded-lg border border-sky-500/30">
            ...
            <p className="text-2xl font-bold text-sky-400">{upcomingFlights.length}</p>
          </div>
      ...
      {flights.length > 0 ? (
        <div className="space-y-3">
          {flights.map((flight) => {
            ...
            return (
              <Card key={flight.id} className={`p-4 border-l-4 ${statusConfig?.color || "border-border"}`}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <Badge variant={flight.status === "delayed" ? "destructive" : "default"} className="text-xs">
                        {statusConfig?.label}
                      </Badge>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-4 p-4 bg-black/20 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-foreground">{flight.departure.airport}</p>
                    <p className="text-sm text-muted-foreground">{flight.departure.city}</p>
                    <p className="text-sm font-medium text-foreground mt-1">{flight.departure.time}</p>
                    <p className="text-xs text-muted-foreground">{flight.departure.date}</p>
                    {flight.departure.gate && (
                      <p className="text-xs text-primary mt-1">Gate {flight.departure.gate}</p>
                    )}
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-lg font-bold text-foreground">{flight.arrival.airport}</p>
                    <p className="text-sm text-muted-foreground">{flight.arrival.city}</p>
                    <p className="text-sm font-medium text-foreground mt-1">{flight.arrival.time}</p>
                    <p className="text-xs text-muted-foreground">{flight.arrival.date}</p>
                    {flight.arrival.gate && (
                      <p className="text-xs text-primary mt-1">Gate {flight.arrival.gate}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 text-xs mb-3">
                  ...
                  <div className="p-2 bg-black/20 rounded-lg">
                    ...
                    <p className="font-mono text-foreground">{flight.confirmationCode}</p>
                  </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {STATUSES.map((status) => (
                    <button
                      key={status.value}
                      onClick={() => handleUpdateStatus(flight.id, status.value as Flight["status"])}
      ...
      <Card className="border-blue-500/20 bg-blue-950/20 p-4">
        <h4 className="font-medium text-sm text-blue-400 mb-3">✈️ Flight Tips</h4>
        <ul className="text-xs text-muted-foreground space-y-2">
          <li>• Check in online 24 hours before departure</li>
          <li>• Arrive 2-3 hours before domestic, 3-4 hours before international</li>
          <li>• Enable flight status notifications on airline app</li>
          <li>• Have confirmationcode ready for check-in</li>
          <li>• Monitor gate changes closely (often announced 30 mins before)</li>
        </ul>
      </Card>
    </div>
  );
