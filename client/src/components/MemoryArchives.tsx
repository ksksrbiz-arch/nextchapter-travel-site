...
import {
  ArchiveIcon, MapPin, Calendar, Users, DollarSign, Zap, Search,
  ChevronRight, Clock, TrendingUp, Globe, Heart, Briefcase
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import memoryArchives from '@/_core/services/memoryArchives';
import type { ArchivedMemory, TripSearchMatch } from '@/_core/services/memoryArchives';

interface MemoryArchivesProps {
  userId?: string;
}

export default function MemoryArchives({ userId = 'user-123' }: MemoryArchivesProps) {
  ...
  const stats = archives.length > 0
    ? {
        ...
        avgMood: (archives.reduce((sum, a) => sum + a.stats.avgDailyMood, 0) / archives.length * 100).toFixed(0),
      }
    : null;
    ...
    <div className="space-y-8">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatBox label="Trips Archived" value={stats.totalTrips} icon={<Globe className="w-6 h-6 text-blue-400" />} />
          <StatBox label="Total Days" value={stats.totalDays} icon={<Calendar className="w-6 h-6 text-blue-400" />} />
          <StatBox label="Memories" value={stats.totalMedia} icon={<Heart className="w-6 h-6 text-blue-400" />} />
          <StatBox label="Avg. Happiness" value={`${stats.avgMood}%`} icon="😊" />
        </div>
      )}
      ...
      {filteredArchives.length === 0 ? (
        <div className="text-center py-12 bg-gray-900/50 rounded-lg border border-gray-800">
          ...
          <p className="text-gray-500 text-sm mt-1">Complete a trip to start building your memory archive</p>
        </div>
      ) : viewMode === 'grid' ? (
            ...
            >
              <div className="relative w-full aspect-video bg-gradient-to-br from-gray-800 to-gray-900 overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-6xl opacity-30">{emotionEmoji[memory.emotionalSummary]}</div>
                </div>
                <div className="absolute inset-0 bg-black/40 group-hover:bg-black/50 transition flex items-end justify-between p-3">
                  <Badge className="bg-blue-500/80 border-0">{memory.stats.duration} days</Badge>
                  <div className="text-right">
                    <Badge className="bg-green-500/80 border-0 mb-1 block">{memory.stats.totalMedia} media</Badge>
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-3">
          {filteredArchives.map((memory, idx) => (
            ...
            >
              <div className="text-4xl">{emotionEmoji[memory.emotionalSummary]}</div>
          ...
          <div className="bg-gray-900 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-gray-900 border-b border-gray-800 p-4 flex items-center justify-between">
              <h2 className="text-2xl font-bold flex items-center gap-2">
                <span className="text-3xl">{emotionEmoji[selectedMemory.emotionalSummary]}</span>
            ...
            <div className="p-6 space-y-6">
              {/* Trip Details */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <DetailBox icon={MapPin} label="Destination" value={selectedMemory.destination} />
                <DetailBox icon={Calendar} label="Duration" value={`${selectedMemory.stats.duration} days`} />
                <DetailBox icon={Heart} label="Media Count" value={selectedMemory.stats.totalMedia.toString()} />
                <DetailBox icon={Zap} label="Mood" value={`${(selectedMemory.stats.avgDailyMood * 100).toFixed(0)}%`} />
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {selectedMemory.archives.map(archive => (
                    <div key={archive.id} className="bg-gray-800/50 rounded p-3 flex items-center justify-between">
                      <div>
                        <p className="font-medium">{archive.name}</p>
                        <p className="text-sm text-gray-400">
                          {archive.metadata.imageCount} photos
                          {archive.metadata.videoCount > 0 && ` • ${archive.metadata.videoCount} videos`}
                        </p>
                      </div>
                      <Badge>{(archive.metadata.totalSize / 1000000).toFixed(1)} MB</Badge>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {similarTrips.map(trip => (
                      <div key={trip.memoryId} className="bg-gray-800/50 rounded p-3">
              ...
              <Button className="w-full">Browse Rebooking Recommendations</Button>
            </div>
          </div>
        </div>
      )}
