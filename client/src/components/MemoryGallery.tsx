...
import {
  Image, Video, Heart, Share2, Trash2, Download,
  ChevronRight, ChevronLeft, X, Calendar, MapPin, Zap
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import memoryCuration from '@/_core/services/memoryCuration';
import type { TripMedia, HighlightMoment } from '@/_core/services/memoryCuration';
...
export default function MemoryGallery({ tripId }: MemoryGalleryProps) {
  ...
  React.useEffect(() => {
    const mockMedia: Array<Omit<TripMedia, 'emotion' | 'confidence'>> = [
      ...
      },
      {
        ...
        location: { name: 'Big Sur', lat: 36.2704, lng: -121.8050 },
        metadata: { brightness: 0.90, blur: false, people: 1 },
      },
      {
        id: 'video-1',
        type: 'video',
        ...
        metadata: { duration: 45, brightness: 0.80, blur: false, people: 0 },
      },
    ];
  ...
  const displayMedia = filteredView === 'highlights'
    ? photos.filter(p => highlights.some(h => h.mediaId === p.id))
    : photos;

  const handlePrevious = () => {
    setSelectedIndex(prev =>
      prev === null ? displayMedia.length - 1 : (prev === 0 ? displayMedia.length - 1 : prev - 1)
    );
  };
  ...
  const selectedPhoto = selectedIndex !== null ? displayMedia[selectedIndex] : null;
    ...
    <div className="space-y-6">
            ...
            <div className="w-full bg-white/10 backdrop-blur-sm p-4 rounded-lg">
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <p className="text-white font-semibold">{selectedPhoto.caption}</p>
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  className={liked.has(selectedPhoto.id) ? 'bg-red-500 border-red-500' : ''}
                >
                  <Heart
                    className={`w-4 h-4 ${
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            {displayMedia.length} moments captured • {photos.filter(p => p.type === 'video').length} videos
          </p>
        </div>
      ...
      {displayMedia.length === 0 ? (
        <div className="text-center py-12 bg-gray-900/50 rounded-lg border border-gray-800">
          ...
          <p className="text-gray-400">No {filteredView === 'highlights' ? 'highlight' : ''} photos yet</p>
          <p className="text-gray-500 text-sm mt-1">Upload or select memories to create albums</p>
        </div>
      ) : (
        <>
          {/* Photo Grid */}
