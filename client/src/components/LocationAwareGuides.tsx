...
import { getRealtimeSyncService, LocationUpdate } from '../_core/services/realtimeSync';
...
const LocationAwareGuides: React.FC<{ tripId: string }> = ({ tripId }) => {
  ...
  const [selectedGuide, setSelectedGuide] = useState<LocationGuide | null>(null);
  ...
  const updateNearbyGuides = (loc: LocationState): void => {
    let guides = MOCK_GUIDES.map((guide) => ({
      ...guide,
      distance: calculateDistance(loc.latitude, loc.longitude, guide.latitude, guide.longitude)
    }))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-cyan-500 rounded-lg p-6 text-white">
        ...
        {loading && (
          <div className="mt-4 text-sm opacity-75">Getting your location...</div>
        )}
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          ...
          className={`px-3 py-1 rounded-full text-sm transition ${
            filterCategory === null ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700'
          }`}
        >
          All
        </button>
        {['restaurant', 'attraction', 'activity', 'emergency'].map((cat) => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-sm transition capitalize ${
              filterCategory === cat ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700'
            }`}
          >
            {cat}
          </button>
            ...
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="text-right">
                  <div className="font-semibold text-blue-600">{guide.distance.toFixed(1)} km</div>
                  {guide.price && <div className="text-sm text-gray-600">{guide.price}</div>}
                </div>
              </div>
        ...
        <div className="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-blue-200 rounded-lg p-6 fixed inset-0 md:static md:inset-auto z-50 md:z-auto flex flex-col md:block">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <div className="text-lg font-semibold">{selectedGuide.distance.toFixed(1)} km</div>
            </div>
