...
import { Users, Plane, TrendingUp, Activity, AlertCircle, Plus, MoreVertical } from 'lucide-react';
...
const ClientListSection: React.FC = () => {
  const [filter, setFilter] = useState<'all' | 'active' | 'vip' | 'at-risk'>('all');
            ...
            {filteredClients.slice(0, 10).map(client => (
              <tr key={client.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                ...
                <td className="py-3 px-4">
                  <span className={`inline-block px-2 py-1 rounded text-xs font-semibold ${statusColor[client.status]}`}>
                    {client.status.charAt(0).toUpperCase() + client.status.slice(1)}
                  </span>
                </td>
                <td className="py-3 px-4 text-gray-600">{client.email}</td>
                <td className="py-3 px-4 text-gray-900 font-semibold">${client.totalSpent.toLocaleString()}</td>
                <td className="py-3 px-4 text-gray-600">{client.tripsBooked}</td>
...
const TripOperationsSection: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-gray-900">Trip Operations ({filteredTrips.length})</h3>
        <div className="flex gap-2">
          {['all', 'planning', 'confirmed', 'in-progress', 'completed'].map(status => (
            <button
              key={status}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filteredTrips.slice(0, 10).map(trip => {
              const daysLeft = Math.ceil((trip.endDate.getTime() - trip.startDate.getTime()) / (1000 * 60 * 60 * 24));
              return (
                <tr key={trip.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                  <td className="py-3 px-4 font-medium text-gray-900">{trip.clientName}</td>
                  <td className="py-3 px-4 text-gray-600">{trip.destination}</td>
                  <td className="py-3 px-4 text-gray-600">
                    {trip.startDate.toLocaleDateString()} - {trip.endDate.toLocaleDateString()}
                    <span className="text-xs text-gray-500 ml-2">({daysLeft}d)</span>
                  </td>
                  <td className="py-3 px-4 text-gray-900 font-semibold">${trip.budget.toLocaleString()}</td>
                  <td className="py-3 px-4 text-gray-900 font-semibold">${trip.expenses.actual.toLocaleString()}</td>
                  <td className="py-3 px-4">
                    <span className={`inline-block px-2 py-1 rounded text-xs font-semibold ${statusColor[trip.status as keyof typeof statusColor]}`}>
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {teamMembers.filter(tm => ['ceo', 'cfo', 'director'].includes(tm.role)).map(member => (
          <div key={member.id} className="border border-gray-200 rounded-lg p-4">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold text-gray-900">{member.performance.tripsManaged}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Satisfaction</span>
                <span className="font-semibold text-gray-900">{member.performance.clientSatisfaction.toFixed(1)}★</span>
              </div>
              <div className="flex justify-between items-center pb-3 border-b border-gray-100">
                <span className="text-sm text-gray-600">Hours Logged</span>
                <span className="font-semibold text-gray-900">{member.performance.hoursLogged}h</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  className="bg-green-600 h-2 rounded-full" 
                  style={{ width: `${(member.performance.clientSatisfaction / 5) * 100}%` }}
                ></div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center pb-4 border-b border-gray-100">
            ...
            <span className="font-semibold text-gray-900">${analytics.avgTripValue.toLocaleString()}</span>
          </div>
          <div className="flex justify-between items-center pb-4 border-b border-gray-100">
            <span className="text-gray-600">Repeat Client Rate</span>
            <span className="font-semibold text-gray-900">{(analytics.repeatClientRate * 100).toFixed(0)}%</span>
          </div>
          <div className="flex justify-between items-center pb-4 border-b border-gray-100">
            <span className="text-gray-600">Growth Rate</span>
            <span className="font-semibold text-green-600">{(analytics.growthRate * 100).toFixed(1)}%</span>
          </div>
          <div className="flex justify-between items-center pb-4 border-b border-gray-100">
            <span className="text-gray-600">Churn Rate</span>
            <span className="font-semibold text-orange-600">{(analytics.churnRate * 100).toFixed(1)}%</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-gray-600">Avg Client Satisfaction</span>
            <span className="font-semibold text-gray-900">{analytics.avgClientSatisfaction.toFixed(1)}★</span>
          </div>
        </div>
      </div>
            ...
            {topDestinations.map((dest, idx) => (
              <div key={idx} className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">${dest.revenue.toLocaleString()} revenue</p>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-gray-900">{dest.count} trips</p>
                </div>
              </div>
            ))}
          </div>
...
const IssuesAlertSection: React.FC = () => {
  const analytics = businessOpsService.generateAnalytics('month');
  const issues = analytics.issues.filter(i => i.severity === 'high').slice(0, 5);
        ...
        {issues.map((issue, idx) => (
          <div key={idx} className="flex items-start gap-3 p-3 bg-white rounded border border-red-100">
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500">Occurred {issue.count}x this month</p>
            </div>
    ...
    <div className="space-y-8 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage clients, trips, and team performance</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          title="Active Trips"
          value={trips.filter(t => ['confirmed', 'in-progress'].includes(t.status)).length}
          icon={<Plane />}
          color="green"
