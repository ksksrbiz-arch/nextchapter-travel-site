...
export function EmptyState({
  ...
  return (
    <div className={`flex flex-col items-center justify-center py-12 px-4 ${className}`}>
      ...
      {description && <p className="text-gray-600 dark:text-gray-400 text-center max-w-sm mb-6">{description}</p>}
...
export function NoResults({ query, className = "" }: NoResultsProps) {
  return (
    <EmptyState
      ...
      description={query ? `No items match "${query}". Try a different search term.` : "No items found."}
      className={className}
    />
  );
}
...
export function LoadingEmptyState({ title, className = "" }: LoadingEmptyStateProps) {
  return (
    <div className={`flex flex-col items-center justify-center py-12 px-4 ${className}`}>
...
import { Calendar, MessageSquare, FileText, BookOpen, AlertCircle, Luggage } from "lucide-react";
...
export function NoPackingItemsEmptyState(props: { onAdd?: () => void }) {
  return (
    <EmptyState
      ...
      action={props.onAdd ? { label: "Add item", onClick: props.onAdd } : undefined}
    />
  );
}
