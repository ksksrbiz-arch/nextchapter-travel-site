...
import {
  Users, TrendingUp, DollarSign, Activity, AlertCircle, CheckCircle,
  Clock, MapPin, Star, Phone, Mail, MoreVertical, Filter, Plus, Eye
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import type { Client, OperationalTrip, BusinessAnalytics } from '@/_core/services/businessOperations';

interface CRMDashboardProps {
  userId?: string;
}

export default function CRMDashboard({ userId = 'user-123' }: CRMDashboardProps) {
  const [clients, setClients] = React.useState<Client[]>([]);
  const [trips, setTrips] = React.useState<OperationalTrip[]>([]);
  const [analytics, setAnalytics] = React.useState<BusinessAnalytics | null>(null);
  const [selectedClient, setSelectedClient] = useState<Client | null>(null);
  const [selectedTrip, setSelectedTrip] = useState<OperationalTrip | null>(null);
  const [view, setView] = useState<'overview' | 'clients' | 'trips' | 'analytics'>('overview');
  ...
  React.useEffect(() => {
    ...
    const mockTrips: OperationalTrip[] = [
      {
        ...
        status: 'confirmed',
        itinerary: [
          { day: 1, activities: ['arrival', 'spa'], accommodation: 'Luxury Resort', notes: 'VIP check-in' },
          { day: 2, activities: ['private guide tour'], accommodation: 'Luxury Resort', notes: '' },
        ],
        assignedTeam: { concierge: 'team-wendy' },
        expenses: { actual: 8500, budgeted: 12000, breakdown: { accommodation: 4200, activities: 3100, meals: 1200 } },
        satisfaction: 0,
        issues: [],
        createdAt: Date.now(),
        updatedAt: Date.now(),
      },
      {
        ...
        status: 'completed',
        itinerary: [
          { day: 1, activities: ['museum tour'], accommodation: 'Boutique Hotel', notes: '' },
          { day: 2, activities: ['cooking class'], accommodation: 'Boutique Hotel', notes: '' },
        ],
        assignedTeam: { guide: 'team-jessica' },
        expenses: { actual: 7900, budgeted: 8500, breakdown: { accommodation: 3500, activities: 2800, meals: 1600 } },
        satisfaction: 5,
        issues: [],
    ...
    setAnalytics({
      ...
      issues: [
        { description: 'Delayed activity confirmations', severity: 'medium', count: 3 },
        { description: 'Client communication delays', severity: 'low', count: 2 },
      ],
    });
    setHealthScore(82);
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <MetricCard icon={Users} label="Active Clients" value={filteredClients.length} color="blue" />
        <MetricCard
          icon={TrendingUp}
          label="Monthly Revenue"
          value={analytics?.totalRevenue ? `$${(analytics.totalRevenue / 1000).toFixed(0)}K` : '-'}
          color="green"
        />
        <MetricCard icon={Activity} label="Active Trips" value={activeTrips} color="purple" />
        <MetricCard
          icon={Star}
          label="Health Score"
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <p className="font-bold">{analytics?.avgClientSatisfaction.toFixed(1)}/5.0</p>
                  </div>
                  <div>
                    <p className="text-gray-400">Repeat Rate</p>
                    <p className="font-bold">{analytics?.repeatClientRate.toFixed(0)}%</p>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  analytics.issues.slice(0, 3).map((issue, idx) => (
                    <div key={idx} className="flex items-start gap-2 text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{issue.count} occurrence(s)</p>
                      </div>
                    </div>
                  ))
                ) : (
                ...
                {analytics.topDestinations.map((dest, idx) => (
                  <div key={idx} className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400">{dest.count} trips • ${dest.revenue}</p>
                    </div>
              ...
              >
                ...
                <div className="space-y-2 text-sm text-gray-400">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    ${client.totalSpent.toLocaleString()} spent • {client.tripsBooked} trips
                  </div>
                </div>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-semibold">{trip.clientName} → {trip.destination}</h4>
                  ...
                  <div className="flex items-center gap-2">
                    {tripStatusIcons[trip.status as keyof typeof tripStatusIcons]}
                ...
                <div className="grid grid-cols-3 gap-4 text-sm text-gray-400 pt-2 border-t border-gray-800">
                  <div>
                    ...
                    <p className="font-semibold text-white">${trip.expenses.actual}/${trip.budget}</p>
                  </div>
                  <div>
      ...
      {view === 'analytics' && analytics && (
        <div className="grid md:grid-cols-2 gap-4">
          <div className="bg-gray-900/50 rounded-lg p-4 border border-gray-800">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">${analytics.avgTripValue.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Growth Rate</span>
                <span className="font-semibold text-green-400">+{analytics.growthRate.toFixed(1)}%</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Churn Rate</span>
                <span className="font-semibold text-red-400">{analytics.churnRate.toFixed(1)}%</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-900/50 rounded-lg p-4 border border-gray-800">
            ...
            <div className="space-y-3">
              {Object.entries(analytics.teamProductivity).map(([name, stats]) => (
                <div key={name}>
                  <div className="flex justify-between text-sm mb-1">
                    <span>{name}</span>
                    <span className="text-xs text-gray-400">{stats.satisfaction}/5.0★</span>
                  </div>
      ...
      {/* Detail Modals */}
      {selectedClient && (
        <DetailModal title={selectedClient.name} onClose={() => setSelectedClient(null)}>
          <div className="space-y-4">
            <div>
            ...
            <div>
              ...
              <p className="text-sm">{selectedClient.tripsBooked} trips • ${selectedClient.totalSpent} total spent</p>
            </div>
      ...
      {selectedTrip && (
        <DetailModal title={`${selectedTrip.clientName} - ${selectedTrip.destination}`} onClose={() => setSelectedTrip(null)}>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-semibold mt-1">${selectedTrip.expenses.actual}/${selectedTrip.budget}</p>
              </div>
            </div>
                ...
                <div className="space-y-2">
                  {selectedTrip.itinerary.map((day, idx) => (
                    <div key={idx} className="text-sm bg-gray-800/50 p-2 rounded">
                      ...
                      <p className="text-xs text-gray-400">{day.activities.join(', ')}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {selectedTrip.issues.map((issue, idx) => (
                    <p key={idx} className="text-sm text-yellow-400">• {issue}</p>
                  ))}
                </div>
              </div>
            )}
...
function MetricCard({
  ...
  return (
    <div className="bg-gray-900/50 rounded-lg p-4 border border-gray-800">
      <div className={`w-10 h-10 rounded bg-gradient-to-br ${colorClass[color]} flex items-center justify-center mb-2`}>
